...
- Wraps every major content block with `1px dashed #1c1c1c` (on dark) or `1px dashed #262626`.
- Vertical padding `40px–80px`.
- Gives the blueprint feeling of a schematic drawing pinned to a drafting board.

### Hero Spotlight Banner
- Carbon background with subtle radial gradient wash fading to Void `#000000` at edges.
- Eyebrow chip in Geist Mono 9px (`1px dashed #ffffff` or `1px solid #ffffff`, 100px radius).
- 70px display headline `Analytics done for you.` in weight 1000.
- Outlined pill action `Book demo`.
- Stippled drafting light wash and client partner logo band (`Operate`, `Chatsheet`, `birk`).

### Feature Split Panels
- 2-column layout (alternating left/right):
  - 12px uppercase mono label (e.g. `CONNECT YOUR DATA`, `QUERY ENGINE`, `DASHBOARDS`)
  - 32px weight 1000 heading
  - 16px body copy
  - 4-item checklist with `#7089ba` Periwinkle check icons
  - 1:1 square CAD wireframe illustration (Gear chassis, CRT terminal, Rocket launcher) with stippled dot matrix.

### 4-Card CAD Wireframe Grid
- Centered section headline: `With setup done, unlock accurate AI-driven analytics.`
- 4 cards with isometric wireframe vector graphics:
  1. `Fast + accurate answers` (Laptop + magic star wand)
  2. `Less reporting work` (Pedestal + `#1` ribbon award)
  3. `Unified data` (Lightning bolt + orbital vortex)
  4. `Drive revenue with data` (Coin cylinder stacks + dollar tokens)

### FAQ Accordion
- 2-column layout with left heading `Frequently asked questions.` and right Radix Accordion with `+` / `−` expand triggers.

### Bottom CTA
- Full-width stippled particle dot grid background with radial light wash.
- Centered headline `Insights without the wait.` and `Book demo` action.
...
### Do
- Use weight 1000 / ultra-bold display for all headlines ≥32px.
- Wrap content in `1px dashed` containment borders.
- Restrict all chromatic color strictly to `#7089ba` (checkmarks, CAD strokes, data nodes).
- Use `100px` fully-pill radius for action buttons.
- Maintain flat matte elevation without drop shadows.

### Don't
- Do not introduce greens, purples, oranges, or rainbow gradients.
- Do not use drop shadows, glows, or blur blurs.
- Do not use solid borders where dashed borders define section containment.
- Do not make buttons with periwinkle fill (actions stay white/dark outlined pills).
